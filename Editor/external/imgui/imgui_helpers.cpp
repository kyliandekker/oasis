...
namespace ImGui
{
	...
	bool CheckboxText(const char* label, bool* p_value, const std::string& checked_text, const std::string& unchecked_text, const ImVec2& size_arg)
	{
		...
		std::string test = (*p_value ? (checked_text + label) : (unchecked_text + label));
		bool temp = false;
		ImGui::Selectable(test.c_str(), &temp, 0, size_arg);
		if (b)
		{
			*p_value = !(*p_value);
		}
	...
	bool InvisInputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
	{
    ...
    bool SearchBar(const char* label, char* buf, size_t buf_size, ImVec2 size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
    {
		float x = ImGui::GetCursorPosX();
		ImGui::SetCursorPosX(x + oasis::imgui::window.SmallFontSize() / 2);
		ImGui::PushFont(oasis::imgui::window.Small());
		...
		float inputTextHeight = initialPadding.y / 2;
		float heightDiff = inputTextHeight / 1.5f;
		ImGui::SetCursorPosY(ImGui::GetCursorPosY() + heightDiff);
		ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(oasis::imgui::window.SmallFontSize() * 2.0f, inputTextHeight));
