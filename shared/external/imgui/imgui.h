...
namespace ImGui
{
	...
	IMGUI_API bool			TreeNodeS(const char* label, const char* id);
    IMGUI_API bool          TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(2);   // helper variation to easily decorelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
    IMGUI_API bool          TreeNode(const void* ptr_id, const char* fmt, ...) IM_FMTARGS(2);   // "
    IMGUI_API bool          TreeNodeV(const char* str_id, const char* fmt, va_list args) IM_FMTLIST(2);
    IMGUI_API bool          TreeNodeV(const void* ptr_id, const char* fmt, va_list args) IM_FMTLIST(2);
    IMGUI_API bool          TreeNodeEx(const char* label, ImGuiTreeNodeFlags flags = 0);
    IMGUI_API bool          TreeNodeExS(const char* label, ImVec2 size = ImVec2(0, 0), ImGuiTreeNodeFlags flags = 0);
...
enum ImGuiInputTextFlags_
{
	...
	ImGuiInputTextFlags_ClickOutsideReturnsTrue = 1 << 21,  // Escape key clears content if not empty, and deactivate otherwise (contrast to default behavior of Escape to revert)
...
enum ImGuiTreeNodeFlags_
{
	...
	ImGuiTreeNodeFlags_OpenOnClick = 1 << 14,   // Need double-click to open node
...
enum ImGuiCol_
{
    ...
    ImGuiCol_Slider,      // Darken/colorize entire screen behind a modal window, when one is active
	ImGuiCol_Link,      // Darken/colorize entire screen behind a modal window, when one is active
	ImGuiCol_Checked,
    ImGuiCol_COUNT
};
